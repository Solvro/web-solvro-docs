---
title: Conventions
description: Konwencje przyjęte podczas tworzenia kodu w projekcie Solvro/web-topwr.
---
...
Each feature is a self-contained module with its own components, types, and utilities. The specific directory layout is listed on [the file structure page](https://docs.solvro.pl/projects/topwr/frontend/file-structure#features-directory-srcfeatures).

<Aside type="note">
  Features need not include all of these files and directories, but only those
...
<Aside type="caution">
  Do not use `process.env` for accessing environment variables directly as they
  are neither typed nor ensured to be present by the configuration.
</Aside>
