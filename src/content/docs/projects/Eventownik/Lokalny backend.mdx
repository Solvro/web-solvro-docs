...
2. Pobierz oficjalny obraz Postgresa za pomocą terminala. W tym przykładzie używamy wersji **17-alpine** - mniej stabilną od **16**, lecz zajmującą mniej miejsca na dysku.
...
- Aby zatrzymać kontener (wyłączyć bazę) wpisz w terminalu `docker stop eventownik-local`.
- Aby uruchomić kontener ponownie, wpisz `docker start eventownik-local` - **start** włącza istniejący kontener, **run** tworzy nowy!
...
Dostosuj plik `.env.local` na froncie wg. lokalnego setupu:
