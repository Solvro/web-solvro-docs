...
Elo żelo, czy zastanawiałeś się kiedyś, jak zrobić formularz feedbacku, który będzie przyjemny dla użytkownika ale też szybki w wykonaniu dla ciebie bez konieczności tworzenia panelu administracyjnego u siebie? W tym poradniku dowiesz się, jak to zrobić z wykorzystaniem Twojego ulubionego frameworku NextJS (może być dowolny inny, nawet zwykły HTML) oraz backendu w postaci Formularzy Google. Zamysł jest taki, żeby stworzyć w pełni właśny formularz na własnej stronie, a odpowiedzi przeglądać na stronie formularzy google czy w arkuszach google.
...
7. **Formularz**
   ...
   :::tip
   Jest to podstawowa metoda tworzenia formularzy z `shadcn/ui` i `react-hook-form`. Jeśli nie rozumiesz tego kodu, zachęcam do odwiedzenia [dokumentacji](https://ui.shadcn.com/docs/components/form) shadcn/ui.
   :::
...
8. **Backend**
   ...
   export const sendFeedbackForm = async (
     ...
     try {
       ...
       formData.append(process.env.FORMS_OPTION1, values.option1);
       formData.append(process.env.FORMS_OPTION2, values.option2);
...
9. **Zdobycie potrzebnych danych**
   ...
   ```
    ...
    FORMS_OPTION1=entry.123456789
    FORMS_OPTION2=entry.987654321
   ```
