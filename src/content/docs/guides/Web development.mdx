...
Czasami w standardowym HTML po kliknieciu TAB lub SPACJI czy ENTER możemy zaobserwować brzydki, gruby ring dookoła przycisku/elementu. Jest to `outline`, który domyślnie jest włączony ale zgódźmy się, pięknie nie wygląda. Jest też w niektórych komponentach `shadcn` (chodź tu pasuje do przycisków czy inputów, chyba, że robimy wariant ghost przycisku i nie chcemy tego).
Oto jak pięknie to wygląda w standardowym CSS:

![Ring focus](https://legacy.reactjs.org/static/dec0e6bcc1f882baf76ebc860d4f04e5/4fcfe/keyboard-focus.png)

Aby to naprawić możemy użyć properties `outline: 0` co na Tailwindowe znaczy mniej więcej tyle:
```jsx
// app/page.tsx
import { Button } from "@/components/ui/button"
...
2. Import the component into your MDX file:
...
Do formatowania dat używamy literek. Każda znaczy co innego. Ważne jest również, czy jest duża czy mała. Do pełej dokumentacji mogę odesłać pod [tym linkiem](https://date-fns.org/v4.1.0/docs/format), ale tutaj jest szybka ściągawka z najpotrzebniejszymi.
...
Ja wiem, że teraz będzie wchodził `React v19` ale zanim to, to minie jeszcze trochę czasu, plus aktualny compiler nie jest doskonały. Ale o co chodzi? Chodzi o to, że jeśli zrobisz coś takiego:
...
to przy każdym zmianie stanu, czy to zmianie w inputcie, kliknięciu, scrollu, strona się rerenderuje i przy okazji wszystkie const typu `welcomeMessage`. Przy tym przykładzie pół biedy, gorzej, jeśli masz jakieś działania matematyczne czy inne rzeczy x10 na jednej stronie. Potrafi to wywalić tak jak Bartusiowi z useEffect. Także aby się przed tym uchronić, możecie użyć hooka o nazwie `useMemo`, który w dużym uproszczeniu, cachuje wam consta i refreshuje się tylko i wyłącznie wtedy, kiedy musi (chodź to też dyskusyjne). Działa dość podobnie jak useEffect, tj posiada argument funkcję, oraz w drugim tablicę zmiennych, na które ma reagować. W naszym przypadku:
