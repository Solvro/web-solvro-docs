...
Teraz co tu się wydarzyło? Przede wszystkim odnieśliśmy się do naszego globalnego obektu `prisma`. Dalej, wybieramy z jakiej tabeli chcemy wyciągnąć dane.
...
`typFinda` definuje w jaki sposób Prisma będzie szukać dane w tabeli. Dostępne opcje to `findMany`, `findFirst` (odpowiada LIMIT 1 w SQL) i `findUnique` (odpowiada DISTINCT w SQL). <br/><br/>
...
- `select` pozwala nam wybrać, które kolumny chcemy zwrócić. Możemy też zwrócić wszystkie kolumny, wogóle nie wpisując `select` do zapytania
