...
{(await fetch("https://cms.solvro.pl/items/Team?fields=name,sociale.link&filter[specialization][Specializations_id][name][_eq]=Backend&filter[sociale][link][_contains]=github")
.then((res) => res.json())
.catch(() => ({ data: [] })))
.data.map((user) => {
const githubUrl = user.sociale.find((social) => social.link.includes("github")).link.trim();
return (

<a href={githubUrl} title={user.name}>
  <Image
    alt={user.name}
