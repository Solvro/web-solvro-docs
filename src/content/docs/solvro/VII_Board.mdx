---
title: VII Zarząd 24/25
sidebar:
  order: 2
description: Roczny plan działania VII Zarządu KN Solvro
---

import {
  Steps,
  Card,
  CardGrid,
  LinkCard,
  Aside,
} from "@astrojs/starlight/components";

## Skład

<CardGrid>
  <Card title="Dawid Linek">
  opis...
  </Card>
  ![](../../../assets/people/dawid_linek.jpg)
<Card title="Szymon Kowaliński">opis...</Card>
Zdjęcie

  <Card title="Wiktoria Mróz"> opis...</Card>
  Zdjęcie
 
</CardGrid>

## Plan działania

<LinkCard
  title="Strategia KN Solvro"
  href=""
  description="Długoterminowy plan strategiczny dyktujący działania KN Solvro."
/>
Z strategii KN Solvro wyznaczyliśmy następujące cele taktyczne:

### S1. Cyfryzacja życia studenckiego PWR

##### Taktyczny S1.T1: Zbudowanie dostępnej żywej bazy wiedzy o Politechnice Wrocławskiej

Głównie za pomocą marki ToPWR chcemy zbudować bazę wiedzy o naszej uczelni.
Otwierając ją na aktualizację przez inne jednostki chcemy stać się najaktualniejszym uporządkowanym źródłem informacji o życiu studenckim PWr.

##### Taktyczny S1.T2: Umożliwienie prostego spersonalizowanego dostępu do tych danych o Politechnice Wrocławskiej

Realizując S1.T1 zdajemy sobie sprawę, że gromadząc ogromne ilości informacji ich przyswojenie przez codzinnego studenta będzie praktycznie niemożliwe.
Tworząc aplikacje, API i innowacyjne rozwiązania AI chcemy zapewnić przystępny dostęp do potrzebnych informacji o PWr.

##### Taktyczny S1.T3: Aplikacje przez studentów dla studentów

Rozwiązania IT są istotnym elementem podczas cyfryzacji życia studenckiego na Politechnice Wrocławskiej.
Głównie aplikacje webowe, mobilne i inne końcowe rozwiązania są używane przez studentów, którym chcemy pomóc.

### S2. Wspomaganie studentów w zakresie IT

##### Taktyczny S2.T1: Regularny cykl wydarzeń

##### Taktyczny S2.T2: Dostępna infrastruktura IT

##### Taktyczny S2.T3: Łącznik w zakresie IT dla innych organizacji

### S3. Stabilny wzrost i działanie KN Solvro

##### Taktyczny S2.T1: Budowa kultury organizacyjnej

##### Taktyczny S2.T2: Przekazanie wiedzy i kompetencji

##### Taktyczny S2.T3: Pozytywny i popularny zewnętrzny obraz KN Solvro

## SWOT

todo

## Kluczowe inicjatywy

todo

## Działania operacyjne

<LinkCard
  title="Cele operacyjne 25Q1"
  href="https://github.com/orgs/Solvro/projects/3/views/7?filterQuery=iteration%3A25Q1"
/>

## Podsumowanie

do uzupełnienia przed końcem kadencji
